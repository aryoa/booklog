<?php
	...
	//echo $output['status'];
	//echo $bodyArray['imageUrl'];
        if (strlen($bodyArray["isbn"]) == 0){
                $output["status"]="2";
                $output["message"]="isbnが指定されていない";
                goto end;
        }
        if (strlen($bodyArray["title"]) == 0){ 
                $output["status"]="2";
                $output["message"]="titleが指定されていない";
                goto end;
        }
	...
	if ($row['IS_EXIST'] == 1){
               $output["status"]="22";
               $output["message"]="isbn='$bodyArray[isbn]'は存在する";
               goto end; 
        }

	// 追加のクエリ発行
	$result_flage = mysql_query("INSERT INTO book VALUES ('$bodyArray[isbn]','$bodyArray[title]','$bodyArray[titleKana]','$bodyArray[author]','$bodyArray[authorKana]','$bodyArray[publisher]','$bodyArray[salesDate]',$bodyArray[itemPrice],'$bodyArray[itemUrl]','$bodyArray[imageUrl]');");
        if(!$result_flage){
                $output["status"]="23";
