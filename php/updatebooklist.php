<?php
	...
	$row = mysql_fetch_assoc($result);
        if ($row['IS_EXIST'] != 1){
               $output["status"]="22";
               $output["message"]="uuid='$bodyArray[uuid]'は存在しない";
               goto end; 
        }

	// 更新のクエリ発行
	$result_flage = mysql_query("UPDATE booklist SET comment='$bodyArray[comment]', satisfaction='$bodyArray[satisfaction]',recorddate='$bodyArray[recorddate]',readdate='$bodyArray[readdate]',unread='$bodyArray[unread]',tag='$bodyArray[tag]' WHERE uuid='$bodyArray[uuid]' AND isbn='$bodyArray[isbn]';");
        if(!$result_flage){
                $output["status"]="25";
                $output["message"]="更新クエリ失敗";
                goto end;
        }
